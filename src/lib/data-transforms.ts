...
export function transformLegacyToV2(legacy: LegacyTool): BaseTool {
  return {
    ...
    pricingSummary: {
      lowestMonthlyPrice:
        legacy.pricing.includes("Free") ||
        legacy.pricing.includes("Open Source")
          ? 0
          : 10,
      highestMonthlyPrice: legacy.pricing.includes("Enterprise")
        ? 100
        : legacy.pricing.includes("Subscription")
          ? 50
          : legacy.pricing.includes("Free")
            ? 0
            : 20,
    ...
    pricingDetails: legacy.pricing.map((tier, index) => ({
      ...
      features: Object.keys(legacy.features).filter(
        (key) => legacy.features[key],
      ),
    ...
    capabilities: {
      ...
      aiFeatures: {
        codeGeneration:
          legacy.features["Code Completion"] ||
          legacy.features["Code Generation"] ||
          false,
      ...
      integrations: {
        ...
        thirdParty: legacy.interface.includes("IDE")
          ? ["VS Code", "JetBrains"]
          : [],
        protocols: ["REST"],
      },
    ...
    semanticTags: [
      ...legacy.functionality.map((f) => f.toLowerCase()),
      ...legacy.tags.primary.map((t) => t.toLowerCase()),
    ],
    aliases: [legacy.name],
