...
export const useTools = (
  ...
  const queryKey = useMemo(() => ["tools", queryParams], [queryParams]);
  ...
  const filterOptions = useMemo(() => {
    ...
    return {
      ...
      interface: Array.from(
        new Set(tools.flatMap((tool) => tool.interface || [])),
      ),
      functionality: Array.from(
        new Set(tools.flatMap((tool) => tool.functionality || [])),
      ),
      deployment: Array.from(
        new Set(tools.flatMap((tool) => tool.deployment || [])),
      ),

      // v2.0 category filters
      primaryCategories: Array.from(
        new Set(tools.flatMap((tool) => tool.categories?.primary || [])),
      ),
      secondaryCategories: Array.from(
        new Set(tools.flatMap((tool) => tool.categories?.secondary || [])),
      ),
      industries: Array.from(
        new Set(tools.flatMap((tool) => tool.categories?.industries || [])),
      ),
      userTypes: Array.from(
        new Set(tools.flatMap((tool) => tool.categories?.userTypes || [])),
      ),

      // v2.0 capability filters
      aiFeatures: Array.from(
        new Set(
          tools.flatMap((tool) =>
      ...
      // v2.0 metadata filters
      status: Array.from(
        new Set(tools.map((tool) => tool.status).filter(Boolean)),
      ),
      complexity: Array.from(
        new Set(
          tools.flatMap(
            (tool) => tool.useCases?.map((useCase) => useCase.complexity) || [],
          ),
        ),
      ),
      pricingModels: Array.from(
        new Set(
          tools.flatMap((tool) => tool.pricingSummary?.pricingModel || []),
        ),
      ),
    };
  }, [tools]);
