...
exports.createEnhancedCollections = main;
 ...
 */
async function createEnhancedCollection(client) {
    const collectionName = enhanced_qdrant_schema_1.enhancedCollectionNames.primary;
    const enabledVectorTypes = (0, enhanced_qdrant_schema_1.getEnabledVectorTypes)();
    try {
        ...
        await client.createCollection(collectionName, enhanced_qdrant_schema_1.defaultEnhancedCollectionOptions);
 ...
 */
async function createLegacyCollections(client) {
    const results = [];
    try {
        console.log(`\n🔧 Creating legacy collections for backward compatibility`);
        const existingCollections = await client.getCollections();
        const existingNames = existingCollections.collections.map(c => c.name);
        for (const [vectorType, collectionName] of Object.entries(database_1.qdrantConfig.collectionNames)) {
            try {
                const vectorConfig = database_1.qdrantConfig.multiVectorsConfig[vectorType];
                if (!vectorConfig) {
 ...
 */
function validateEnhancedConfiguration() {
    try {
        console.log(`🔍 Validating enhanced collection configuration`);
        // Validate vector configurations
        const enabledTypes = (0, enhanced_qdrant_schema_1.getEnabledVectorTypes)();
        ...
        (0, enhanced_qdrant_schema_1.validateEnhancedVectors)(sampleVectors);
 ...
 */
async function displayCollectionInfo(client) {
    try {
...
async function main() {
    ...
    console.log(`   Target: ${database_1.qdrantConfig.host}:${database_1.qdrantConfig.port}`);
    console.log(`   Enhanced collection: ${enhanced_qdrant_schema_1.enhancedCollectionNames.primary}`);
    try {
        ...
        const client = await (0, database_1.connectToQdrant)();
...
//# sourceMappingURL=create-enhanced-collections.js.map