...
export class MCPToolConverter {
  ...
  static async convertAllTools(): Promise<DynamicStructuredTool[]> {
    ...
    if (mcpTools.length === 0) {
      console.warn('[MCPToolConverter] No MCP tools available. Make sure MCP client is connected.');
      return [];
    }
    
    console.log(`[MCPToolConverter] Converting ${mcpTools.length} MCP tools to LangChain format`);
    
    const langchainTools = mcpTools.map(tool => {
      try {
        return this.convertTool(tool);
      } catch (error) {
        console.error(`[MCPToolConverter] Failed to convert tool ${tool.name}:`, error);
        return null;
      }
    }).filter((tool): tool is DynamicStructuredTool => tool !== null);
    
    console.log(`[MCPToolConverter] Successfully converted ${langchainTools.length} tools`);
   ...
   */
  static getToolInfo(): Array<{ name: string; description: string; schema: any }> {
    const mcpTools = mcpClient.getAvailableTools();
