  ...
  @Prop({ 
    ...
    validate: {
      validator: (v: string[]) => {
        ...
        return v.every(keyword => typeof keyword === 'string' && keyword.length <= 256);
      },
      message: 'searchKeywords must be a non-empty array with strings max 256 chars each'
    }
  })
  ...
  @Prop({
    ...
    validate: {
      validator: function(v: { primary: string[], secondary: string[] }) {
        ...
        const hasNonEmptyPrimary = Array.isArray(v.primary) && v.primary.length > 0;
        const hasNonEmptySecondary = Array.isArray(v.secondary) && v.secondary.length > 0;
        return hasNonEmptyPrimary || hasNonEmptySecondary; // At least one must be non-empty
      },
      message: 'tags must have at least one non-empty array (primary or secondary)'
    }
  })
  tags!: {
    primary: string[];
...
ToolSchema.index({ 'tags.secondary': 1 }, { name: 'tool_tags_secondary_index' }); // Filter by secondary tags

// Compound indexes for common query patterns
ToolSchema.index({ 
  functionality: 1, 
  rating: -1 
}, { name: 'tool_functionality_rating_index' }); // Functionality with rating

ToolSchema.index({ 
  deployment: 1, 
  popularity: -1 
}, { name: 'tool_deployment_popularity_index' }); // Deployment with popularity